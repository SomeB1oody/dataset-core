...
`Dataset<T>` 设计成被封装使用。配套 crate [`dataset-ml`](https://crates.io/crates/dataset-ml) 展示了推荐的模式；以下是一个简化的大纲：
...
- **跨线程安全**：只要 `T` 是 `Send + Sync`，`Dataset<T>` 就是 `Send + Sync`；内部 `OnceLock` 保证即使在并发调用下加载器也最多执行一次。
