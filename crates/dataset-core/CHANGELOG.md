...
- `gunzip` (feature `utils`): decompress a `.gz` file into a single output. It streams the data, so the whole file never sits in memory at once. This adds the pure-Rust `flate2` dependency.
...
### Added
- `set_loader`, `invalidate`, `into_inner` / `take`, and `get` / `get_mut`: these replace the stored loader, drop the cached value, move the cached value out, or borrow or edit it in place without triggering a load.
