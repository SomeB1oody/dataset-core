...
- **Breaking:** these two variants extend `DataFormatErrorKind`. A `match` over that enum that lists every variant no longer compiles. Add one arm per new variant, or add a `_` arm.
