...
## [Unreleased]
### Added
- `traits::MlDataset`, a trait implemented by **every** loader in this crate (re-exported at the crate root as `dataset_ml::MlDataset`). Until now nothing could be written generically over "some dataset": each loader's inherent accessors are named for what it actually holds (`features()`/`labels()`, `targets()`, `texts()`), which is why they read well but also why a function had to name one concrete struct. `MlDataset` is the common denominator — the container operations that are the same whatever a loader parses into — with an associated `Data` type and a `NAME` constant. It also closes three real gaps in the public API: **`invalidate()`** (drop the in-memory cache, so the memory a large dataset holds can be reclaimed, or a changed file picked up), **`is_loaded()`** / **`storage_dir()`** (inspect a loader without touching the data), and **`n_samples()`** (a uniform sample count). Its data accessors are deliberately named `load` / `load_mut` / `peek` / `unload` rather than reusing `data` / `get_data` / `get_data_mut` / `take_data`, so a trait method never silently shadows — or is shadowed by — the inherent method of the same name; both sets stay available and always agree. Implementors supply only `dataset()`, `dataset_mut()`, and `into_dataset()`; everything else is provided. A companion `NumSamples` trait (also re-exported at the crate root) is implemented blanket-style for the array pairs and triples the loaders produce, and is the associated-type bound that makes `n_samples()` callable from a generic function without restating it.
- `preprocessing`, a module of helpers that turn what the loaders return into what a model consumes, so a baseline no longer requires hand-rolling the usual glue or pulling in a framework. It adds **no dependencies**: shuffling uses a SplitMix64 generator built into the module, seeded explicitly, so the same seed and inputs reproduce the same split on every platform. It provides `train_test_split`, `stratified_split` (preserving each class's proportion — it matters for `sms_spam`, `covtype`, and `kddcup99`, where an unstratified split can drop a rare class from the test set entirely), `k_fold_indices`, `shuffled_indices`, `standardize` / `min_max_scale` / `apply_scaler` (plus the `Scaler` struct they fit, so a scaler fitted on the training rows can be replayed on the test rows instead of refitted), `one_hot_encode` for the categorical `Array2<String>` the mixed-type and all-categorical loaders return, and `label_encode` / `class_counts`. Two design points: the splitting functions return **row indices** (the `IndexSplit` alias) rather than arrays, because a sample is spread across two or three parallel arrays and one index list keeps them aligned — materialize with ndarray's `select(Axis(0), &indices)`; and the scalers compute their statistics over the **finite** values of each column, leaving non-finite entries untouched, so the `NaN` marking a missing value in `titanic` / `palmer_penguins` / `heart_disease` stays missing rather than poisoning the whole column.
- `spambase::Spambase` loader for the Spambase dataset (UCI Machine Learning Repository, Hopkins, Reeber, Forman & Suermondt 1999): 4,601 emails (2,788 ham, 1,813 spam), 57 numeric features, and an `Array1<&'static str>` label of `"ham"`/`"spam"` (mapped from the source's `0`/`1`, matching the convention `SmsSpam`/`YoutubeSpam` already use) for binary classification. It is the **feature-engineered counterpart** to the crate's raw-text spam corpora: the vectorization `SmsSpam` and `YoutubeSpam` leave to the caller is already done here, so `SpambaseData = (Array2<f64>, Array1<&'static str>)` drops straight into a numeric model. The 57 features are 48 `word_freq_*` percentages, 6 `char_freq_*` percentages, and 3 capital-run-length statistics. Sourced from a **ZIP archive** (like `digits`), using `spambase.data` (cached as `spambase.csv`) with SHA-256 verification of the extracted file. The struct is also re-exported at the crate root as `dataset_ml::Spambase`.
- `letter_recognition::LetterRecognition` loader for the Letter Recognition dataset (UCI Machine Learning Repository, Slate 1991): 20,000 samples, 16 integer features in `0..=15` describing a distorted glyph's pixel image, and the capital letter it shows as the target — the crate's **widest classification problem by class count** (26 classes) and its largest multi-class dataset after `covtype`/`kddcup99`. It is the first loader whose label is an `Array1<char>`: a one-letter class is naturally a `char`, so `LetterRecognitionData = (Array2<f64>, Array1<char>)` needs no 26-entry lookup table and `labels[i] == 'Q'` reads as the task does. Like `mushroom`, the label is the **first** source column. Sourced from a **ZIP archive**, using `letter-recognition.data` (cached as `letter_recognition.csv`) with SHA-256 verification. The struct is also re-exported at the crate root as `dataset_ml::LetterRecognition`.
- `banknote_authentication::BanknoteAuthentication` loader for the Banknote Authentication dataset (UCI Machine Learning Repository, Lohweg 2012): 1,372 samples and 4 continuous features (`variance`, `skewness`, `curtosis` — the UCI spelling is kept — and `entropy` of Wavelet-transformed banknote images), the crate's most compact pure-numeric benchmark. `BanknoteAuthenticationData = (Array2<f64>, Array1<u8>)`: the target is the source's **raw `0`/`1` code** rather than `"genuine"`/`"forged"` names, because UCI does not document which code corresponds to which, and mapping them would risk mislabelling the dataset. There are no missing values. Sourced from a **ZIP archive** holding a single `data_banknote_authentication.txt` (cached as `banknote_authentication.csv`) with SHA-256 verification. The struct is also re-exported at the crate root as `dataset_ml::BanknoteAuthentication`.
- `DOWNLOAD_RETRIES`, the crate's shared download-retry policy (2 extra attempts), exposed as a public constant.

### Changed
- Every loader now fetches through `dataset-core`'s new `download_to_with_retries` instead of `download_to`, with `DOWNLOAD_RETRIES` extra attempts. The datasets live on university archives and personal pages that intermittently time out or reset a connection, and a run failing for that reason is not a bug in the data. Errors retrying cannot fix are still returned immediately, and a genuinely unreachable host costs at most 1.5 s of waiting before failing.
- `wine_quality::WineData` is now `pub` rather than `pub(crate)`, so that `RedWineQuality` and `WhiteWineQuality` can name it as their `MlDataset::Data`. It is unchanged as a type: `(Array2<f64>, Array1<f64>)`.

### Testing
- Added `tests/preprocessing_test.rs` (27 tests) and `tests/traits_test.rs` (6 tests). The preprocessing suite needs no network — it checks each function against hand-built arrays, including that splits are permutations and are seed-reproducible, that stratification keeps a 2%-of-the-data class present in the test set, that a singleton class goes to the train side, that k-folds validate each sample exactly once, that scalers ignore and preserve `NaN`, that a constant column maps to zeros rather than dividing by zero, and that a scaler replayed on new data uses the training statistics.
- `tests/common/mod.rs` now delegates to `dataset-core`'s new `verify_sha256` rather than reimplementing SHA-256 hashing, so the tests exercise the same code path the loaders use during acquisition. This removes the `sha2` dev-dependency.

## [0.3.0] - 2026-07-12
### Added
- `movie_review_polarity::MovieReviewPolarity` loader for the Cornell Movie Review Polarity dataset (polarity dataset v2.0, Pang & Lee 2004): 2,000 full IMDb movie reviews (1,000 positive, 1,000 negative — balanced) for binary document-level sentiment classification. It is a **text** loader complementing the sentence-level `SentimentSentences` with whole-document reviews: `MovieReviewPolarityData` is `(Array1<String>, Array1<&'static str>)`, `texts()` returns the pre-tokenized, lowercased reviews and `labels()` returns an `Array1<&'static str>` of `"positive"`/`"negative"` (from the `pos`/`neg` folders). Like `Newsgroups20` it is sourced from a `.tar.gz` (decompressed with `untar_gz`) and, since the reviews are multi-line raw documents, caches the tarball as-is (its SHA-256 is the integrity check) and re-extracts it in memory on load, decoding each file as **Latin-1** and walking `neg` then `pos` in lexicographic file order. The struct is also re-exported at the crate root as `dataset_ml::MovieReviewPolarity`.
- `newsgroups20::Newsgroups20` loader for the classic 20 Newsgroups dataset (Lang 1995; the `bydate` version curated by Jason Rennie — the same tarball scikit-learn's `fetch_20newsgroups` downloads): ~18,846 Usenet posts across 20 newsgroups for **multi-class** text classification. It is the crate's first **multi-class text** loader and the framework-agnostic analogue of `fetch_20newsgroups`: `Newsgroups20Data` is `(Array1<String>, Array1<&'static str>)`, `texts()` returns the full raw posts (including the email-style headers — nothing stripped, matching scikit-learn's default) and `labels()` returns an `Array1<&'static str>` of the 20 newsgroup names (e.g. `"sci.space"`). Mirroring scikit-learn's `subset` argument, three constructors share one cached archive: `new` (train, 11,314 posts — the default), `new_test` (test, 7,532), and `new_all` (18,846). It is the first loader sourced from a `.tar.gz`, decompressed with `dataset-core`'s new `untar_gz` helper; unlike the other text loaders (which cache a combined CSV) the posts are multi-line raw documents, so the loader caches the tarball as-is (its SHA-256 is the integrity check) and re-extracts it in memory on load, decoding each file as **Latin-1** (like scikit-learn) so non-UTF-8 bytes are preserved. Samples are walked in a deterministic category-then-file lexicographic order. Adds `tempfile` as a direct dependency (for the temporary extraction directory). The struct is also re-exported at the crate root as `dataset_ml::Newsgroups20`.
- `sentiment_sentences::SentimentSentences` loader for the Sentiment Labelled Sentences dataset (UCI Machine Learning Repository, Kotzias, Denil, de Freitas & Smyth 2015): 3,000 review sentences (1,000 each from Amazon, IMDb, and Yelp; 1,500 positive, 1,500 negative — 500 of each per site, so it is perfectly balanced) for binary sentiment classification. It is the crate's **third text loader** and the first to carry per-sample **metadata**: besides `texts()` (a `(3000,)` `Array1<String>` of raw sentences) and `labels()` (an `Array1<&'static str>` of `"positive"`/`"negative"`, mapped from the source labels `1`/`0`), it adds a **`sources()`** accessor returning an `Array1<&'static str>` of `"amazon"`/`"imdb"`/`"yelp"` (the review site each sentence came from — useful for slicing by domain or cross-domain transfer). Because of that extra column, `SentimentSentencesData` is a **triple** `(Array1<String>, Array1<&'static str>, Array1<&'static str>)` and `data()` returns all three. It is sourced from a **ZIP archive** of three per-site `sentence<TAB>label` files that carry no source column; the loader tags each line with its site and combines them (in a fixed order) into a single `source<TAB>sentence<TAB>label` corpus (cached as `sentiment_sentences.csv`, covered by one pinned SHA-256), then parses it tab-separated with csv quote handling **disabled** (`quoting(false)`, like `SmsSpam`). The struct is also re-exported at the crate root as `dataset_ml::SentimentSentences`.
